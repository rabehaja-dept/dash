...
import { SANITY_BASE_URL, SANITY_ALGOLIA_INDEX } from "~/lib/consts";
...
type PagePayload = {
  // from the sanity / algolia webhook handler
  title: string;
  path: string;
  body: string;
};
...
type HitProps = {
  hit: AlgoliaHit<PagePayload>;
};

function Hit({ hit }: HitProps) {
  return (
    <Link href={`${SANITY_BASE_URL}/${hit.path}`}>
  ...
  return (
    <InstantSearchSSRProvider {...serverState}>
      ...
      >
        ...
        <InstantSearch
          searchClient={client}
          indexName={SANITY_ALGOLIA_INDEX}
          routing={{
