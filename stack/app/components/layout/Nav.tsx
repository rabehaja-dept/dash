...
import { Link, NavLink } from "@remix-run/react"; // @dash-replace contentful: import { Link, NavLink } from "@remix-run/react";
...
export function Nav({ sticky = false, banner, slot, children }: NavProps) {
  ...
  const links = [
    { name: t("Home"), href: "/" },
    { name: "Contentful", href: "/contentful" }, // @dash-remove contentful
    { name: "Strapi", href: "/strapi-blog" }, // @dash-remove strapi
    { name: "Kontent.ai", href: "/kontent.ai" }, // @dash-remove kontent.ai
    { name: "Contentstack", href: "/contentstack" }, // @dash-remove contentstack
    { name: t("Shopify"), href: "/shopify" }, // @dash-remove shopify
    { name: t("Commercetools"), href: "/commercetools" }, // @dash-remove commercetools
    { name: t("Search"), href: "/contentful/search" }, // @dash-remove contentful
  ];
      ...
      <div className="border-1 flex h-[80px] items-center justify-between border border-border-weak bg-white px-10">
        ...
        <div className="flex items-center">
          ...
          <motion.span
            ...
            className="mr-10 hidden xl:inline"
          >
            {links.map(({ name, href }) => (
              <NavLink
                key={name}
          ...
          <div className="ml-10 flex items-center">
            ...
            <div className="mt-1">
              <Link to="/commercetools/account" title="Account">
                <UserCircleIcon className="h-8 w-8 hover:opacity-60" />
              </Link>
            </div>
            {/* @dash-remove-end */}
            {slot}
          ...
          <Popover.Button
            className="ml-4 flex w-10 cursor-pointer items-center align-top xl:hidden"
